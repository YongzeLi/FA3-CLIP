...
class TextPart(nn.Module):
    ...
    def forward(self, XY_R, hq_XY_R=None, split='train'):
        ...
        prompts_fake, prompts_live, tokenized_prompts_fake, tokenized_prompts_live = self.prompter_learn(total_prompt_fake, total_prompt_live, split=split) # 训练时prompt [B/2, 77, 512]tokenized[B/2, 77]
...
class VisionPart(nn.Module):
    ...
    def forward(self, img_cls: torch.Tensor, x: list, x_ori: torch.Tensor):
        ...
        fea_stm = self.StmBlock(all_freq)  # Frequency Compression
