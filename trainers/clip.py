...
class CLIP(TrainerFAS):
    ...
    def build_model(self):
        ...
        self.model.VisionPart = VisionPart(cfg, vision_dim, embed_dim)
    ...
    def forward_VL(self, clip_model, XY_R, split=None):
        ...
        image_features, fea_vision_each_block = clip_model.encode_image(XY_R)  # image features, frequency features for all layers
        image_features = clip_model.VisionPart(image_features, fea_vision_each_block, XY_R)
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        
        hq_XY_R = clip_model.highFrequencyExtractor(XY_R)
        text_features_fake, text_features_live = clip_model.TextPart(XY_R, hq_XY_R, split)  # text features
